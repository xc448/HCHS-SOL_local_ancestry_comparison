...
```{r}
blacklist <- read.table("./hg38-blacklist.bed" , header = FALSE, sep = "\t")
...
plot_mapped_low_correlation <- function(dat, chrnum, distance = 500000, cor_thresh = 0.9){
  ...
  blacklist_chr <- blacklist |> filter(Chromosome == chrnum) 
  if(nrow(dat_chr) > 0) {
    filtered_intervals <- filter_intervals(blacklist_chr, dat_chr)
    g <- dat_chr |>
...
```{r}
blacklisted_intervals <- c()
plots <- list()
for(chr in 1:22){
  res <-  plot_mapped_low_correlation(dat_tot, chr)
  plots[[chr]] <- res$g 
  blacklisted_intervals <- rbind(blacklisted_intervals, res$filtered_intervals)
}
```
...
## Add SNPs mapped to the HLA regions chr6:28510120:33480577 to the mapped list 
```{r}
startpos <- 28510120-500000 # the start position in HLA - 0.5 Mb
endpos <- 33480577+500000 # the end position in HLA + 0.5 Mb
HLA <- dat_tot[dat_tot$Chromosome == 6 & (dat_tot$Position %in% startpos:endpos), 
               c("Chromosome", "Position", "rsID")] 
HLA <- HLA |> mutate(startpos= rep(28510120, nrow(HLA)), 
                     endpos =rep(33480577, nrow(HLA)),
                     note = rep("HLA region", nrow(HLA))) |>
  select(Chromosome, startpos, endpos, note, Position, rsID)

blacklisted_intervals <- rbind(blacklisted_intervals, HLA)
length(unique(blacklisted_intervals$rsID))
```

## Add SNPs mapped to the IGH regions chr15:105586437:106879844 to the mapped list 
```{r}
startpos <- 105586437-500000 # the start position in IGH - 0.5 Mb
endpos <- 106879844+500000 # the end position in IGH + 0.5 Mb
IGH <- dat_tot[dat_tot$Chromosome == 14 & (dat_tot$Position %in% startpos:endpos), 
               c("Chromosome", "Position", "rsID")] 
IGH <- IGH |> mutate(startpos= rep(105586437, nrow(IGH)), 
                     endpos =rep(106879844, nrow(IGH)),
                     note = rep("IGH region", nrow(IGH))) |>
  select(Chromosome, startpos, endpos, note, Position, rsID)

blacklisted_intervals <- rbind(blacklisted_intervals, IGH)
length(unique(blacklisted_intervals$rsID))
```

Unique blacklisted intervals that are in proximal to the low correlation regions as reported in the supplementary table
```{r}
supp_table <- blacklisted_intervals |> group_by(Chromosome, startpos, endpos, note) |>
  group_keys()
```

## Calculate the proportion of the mapped SNPs among all SNPs with low correlations: 0.9179402
```{r}
length(unique(blacklisted_intervals$rsID))/nrow(tot_low_region)
```
