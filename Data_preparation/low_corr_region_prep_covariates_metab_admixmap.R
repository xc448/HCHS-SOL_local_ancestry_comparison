...
metab_vals_batch1 <- merge(metab_vals_batch1, 
                           metab_vals_batch1_raw[,c("propyl4hydroxybenzoatesulfate_std", "cysglyoxidized_std",
                                                    "LAB_ID")], 
...
metab_vals_batch1 <- metab_vals_batch1 |>
  ...
  mutate(ranks = rank(cysglyoxidized_std, ties.method = "average"),
         int_norm = qnorm((ranks - 0.5) / max(ranks, na.rm = TRUE))) |>
...
# propyl 4-hydroxybenzoate sulfate 100006264 
...
allinfo_model_b1 <- merge(allinfo_batch1, flare_b1, by = "SUBJECT_ID")
...
allinfo_model_b2 <- merge(allinfo_batch2, flare_b2, by = "SUBJECT_ID")
